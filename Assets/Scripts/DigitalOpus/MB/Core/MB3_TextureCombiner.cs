	...
	public class MB3_TextureCombiner
	{
			...
			public void CalcInitialFullSamplingRects(bool fixOutOfBoundsUVs)
			{
				...
				for (int i = 0; i < ts.Length; i++)
				{
					if (ts[i].t != null)
					{
						...
						DRect r2 = fixOutOfBoundsUVs ? obUVrect : new DRect(0.0, 0.0, 1.0, 1.0);
		...
		private bool _CollectPropertyNames(Material resultMaterial, List<ShaderTextureProperty> texPropertyNames)
		{
			...
			for (int j = 0; j < shaderTexPropertyNames.Length; j++)
			{
				...
				{
					str = str + ", " + shaderTexPropertyNames[j].name;
			...
			for (int k = 0; k < _customShaderPropNames.Count; k++)
			{
				...
				{
					str = str + ", " + _customShaderPropNames[k].name;
			...
			try
			{
				...
				else
				{
					for (int i = 0; i < objsToMesh.Count; i++)
					{
						...
						foreach (Material x in gOMaterials)
						{
							if (x == null)
							{
		...
		private IEnumerator __Step1_CollectDistinctMatTexturesAndUsedObjects(ProgressUpdateDelegate progressInfo, CombineTexturesIntoAtlasesCoroutineResult result, List<GameObject> allObjsToMesh, List<Material> allowedMaterialsFilter, List<ShaderTextureProperty> texPropertyNames, MB2_EditorMethodsInterface textureEditorMethods, List<MB_TexSet> distinctMaterialTextures, List<GameObject> usedObjsToMesh)
		{
			...
			for (int i = 0; i < allObjsToMesh.Count; i++)
			{
				...
				if (!dictionary.TryGetValue(mesh.GetInstanceID(), out MB_Utility.MeshAnalysisResult[] value))
				{
				...
				{
					UnityEngine.Debug.Log("Mesh Analysis for object " + gameObject + " numSubmesh=" + value.Length + " HasOBUV=" + value[0].hasOutOfBoundsUVs + " UVrectSubmesh0=" + value[0].uvRect);
				}
						...
						if (material.HasProperty(texPropertyNames[l].name))
						{
							...
							if (texture != null)
							{
								...
								if ((format != TextureFormat.ARGB32 && format != TextureFormat.RGBA32 && format != TextureFormat.BGRA32 && format != TextureFormat.RGB24 && format != TextureFormat.Alpha8) || flag2)
								{
									...
									{
										UnityEngine.Debug.LogError("Object " + gameObject.name + " in the list of objects to mesh uses Texture " + texture2D.name + " uses format " + format + " that is not in: ARGB32, RGBA32, BGRA32, RGB24, Alpha8 or DXT. These textures cannot be resized at runtime. Try changing texture format. If format says 'compressed' try changing it to 'truecolor'");
										result.success = false;
										yield break;
									}
			...
			else
			{
				...
				for (int l = 0; l < distinctMaterialTextures.Count; l++)
				{
					...
					for (int m = 0; m < texPropertyNames.Count; m++)
					{
						...
						{
							UnityEngine.Debug.LogWarning("Texture " + meshBakerMaterialTexture.t + "is tiled by " + meshBakerMaterialTexture.matTilingRect.size + " tiling will be baked into a texture with maxSize:" + _maxTilingBakeSize);
						}
						...
						{
							UnityEngine.Debug.LogWarning("Texture " + meshBakerMaterialTexture.t + "has out of bounds UVs that effectively tile by " + mB_TexSet.obUVscale + " tiling will be baked into a texture with maxSize:" + _maxTilingBakeSize);
						}
						...
						if ((int)(adjustedForScaleAndOffset2Dimensions.x * adjustedForScaleAndOffset2Dimensions.y) > num2 * num3)
						{
							if (LOG_LEVEL >= MB2_LogLevel.trace)
							{
								UnityEngine.Debug.Log("    matTex " + meshBakerMaterialTexture.t + " " + adjustedForScaleAndOffset2Dimensions + " has a bigger size than " + num2 + " " + num3);
							}
				...
				for (int propIdx = 0; propIdx < numAtlases; propIdx++)
				{
					...
					else
					{
						...
						for (int texSetIdx = 0; texSetIdx < distinctMaterialTextures.Count; texSetIdx++)
						{
							...
							progressInfo?.Invoke(s + "Copying to atlas: '" + texSet.ts[propIdx].t + "'", 0.02f);
						...
						atlas = new Texture2D(atlasSizeX, atlasSizeY, TextureFormat.ARGB32, mipChain: true);
		...
		private Rect[] __CreateAtlasesMBTexturePackerFast(ProgressUpdateDelegate progressInfo, int numAtlases, List<MB_TexSet> distinctMaterialTextures, List<ShaderTextureProperty> texPropertyNames, bool[] allTexturesAreNullAndSameColor, Material resultMaterial, Texture2D[] atlases, MB2_EditorMethodsInterface textureEditorMethods, int _padding)
		{
			...
			num = rects[0].atlasX;
			num2 = rects[0].atlasY;
			result = rects[0].rects;
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				UnityEngine.Debug.Log("Generated atlas will be " + num + "x" + num2 + " (Max atlas size for platform: " + maxAtlasSize + ")");
			}
				...
				for (int j = 0; j < numAtlases; j++)
				{
					...
					else
					{
						...
						for (int k = 0; k < distinctMaterialTextures.Count; k++)
						{
							...
							{
								progressInfo?.Invoke("Resizing texture '" + texture2D2 + "'", 0.01f);
								if (LOG_LEVEL >= MB2_LogLevel.debug)
								{
		...
		private void MergeOverlappingDistinctMaterialTexturesAndCalcMaterialSubrects(List<MB_TexSet> distinctMaterialTextures, bool fixOutOfBoundsUVs)
		{
			...
			for (int k = 0; k < distinctMaterialTextures.Count; k++)
			{
				...
				DRect obUVRectIfTilingSame = fixOutOfBoundsUVs ? new DRect(mB_TexSet2.obUVoffset, mB_TexSet2.obUVscale) : new DRect(0.0, 0.0, 1.0, 1.0);
			...
			for (int m = 0; m < distinctMaterialTextures.Count; m++)
			{
				...
				for (int n = m + 1; n < distinctMaterialTextures.Count; n++)
				{
					...
					else
					{
						...
						dRect = MB3_UVTransformUtility.GetEncapsulatingRect(ref uvRect, ref uvRect2);
					...
					if (num3 < num4)
					{
						...
						{
							mB_TexSet4.ts[num13].encapsulatingSamplingRect = dRect;
						}
						if (!list.Contains(m))
						{
							list.Add(m);
		...
		private Vector2 GetAdjustedForScaleAndOffset2Dimensions(MeshBakerMaterialTexture source, Vector2 obUVoffset, Vector2 obUVscale)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				UnityEngine.Debug.Log("GetAdjustedForScaleAndOffset2Dimensions: " + source.t + " " + obUVoffset + " " + obUVscale);
			}
		...
		public Texture2D GetAdjustedForScaleAndOffset2(MeshBakerMaterialTexture source, Vector2 obUVoffset, Vector2 obUVscale)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				UnityEngine.Debug.LogWarning("GetAdjustedForScaleAndOffset2: " + source.t + " " + obUVoffset + " " + obUVscale);
			}
		...
		public IEnumerator CopyScaledAndTiledToAtlas(MeshBakerMaterialTexture source, MB_TexSet sourceMaterial, ShaderTextureProperty shaderPropertyName, DRect srcSamplingRect, int targX, int targY, int targW, int targH, bool _fixOutOfBoundsUVs, int maxSize, Color[][] atlasPixels, int atlasWidth, bool isNormalMap, ProgressUpdateDelegate progressInfo = null)
		{
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				UnityEngine.Debug.Log("CopyScaledAndTiledToAtlas: " + source.t + " inAtlasX=" + targX + " inAtlasY=" + targY + " inAtlasW=" + targW + " inAtlasH=" + targH);
			}
			...
			for (int k = 0; k < w; k++)
			{
				...
				for (int l = 0; l < h; l++)
				{
					...
					atlasPixels[targY + l][targX + k] = t2.GetPixelBilinear(u, v);
				}
			}
			for (int m = 0; m < w; m++)
			{
			...
			for (int j = 1; j <= _atlasPadding; j++)
			{
				...
				{
					atlasPixels[targY - i][targX - j] = atlasPixels[targY][targX];
					atlasPixels[targY + h - 1 + i][targX - j] = atlasPixels[targY + h - 1][targX];
					atlasPixels[targY + h - 1 + i][targX + w + j - 1] = atlasPixels[targY + h - 1][targX + w - 1];
					atlasPixels[targY - i][targX + w + j - 1] = atlasPixels[targY][targX + w - 1];
					yield return null;
				}
				yield return null;
			}
		...
		public void SuggestTreatment(List<GameObject> objsToMesh, Material[] resultMaterials, List<ShaderTextureProperty> _customShaderPropNames)
		{
			...
			for (int l = 0; l < objsToMesh.Count; l++)
			{
				...
				for (int m = 0; m < gOMaterials2.Length; m++)
				{
					...
					{
						if (!dictionary2.TryGetValue(gOMaterials2[m], out List<GameObject> value2))
						{
