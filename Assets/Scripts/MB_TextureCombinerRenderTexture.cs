...
public class MB_TextureCombinerRenderTexture
{
	...
	public void OnRenderObject()
	{
		...
		for (int i = 0; i < rs.Length; i++)
		{
			...
			{
				UnityEngine.Debug.Log("Added " + meshBakerMaterialTexture.t + " to atlas w=" + meshBakerMaterialTexture.t.width + " h=" + meshBakerMaterialTexture.t.height + " offset=" + meshBakerMaterialTexture.matTilingRect.min + " scale=" + meshBakerMaterialTexture.matTilingRect.size + " rect=" + rs[i] + " padding=" + _padding);
	...
	private void CopyScaledAndTiledToAtlas(MB3_TextureCombiner.MB_TexSet texSet, MB3_TextureCombiner.MeshBakerMaterialTexture source, Vector2 obUVoffset, Vector2 obUVscale, Rect rec, ShaderTextureProperty texturePropertyName, TextureBlender resultMatTexBlender)
	{
		...
		if (LOG_LEVEL >= MB2_LogLevel.trace)
		{
			UnityEngine.Debug.Log("DrawTexture tex=" + t.name + " destRect=" + rect + " srcRect=" + r + " Mat=" + mat);
		}
