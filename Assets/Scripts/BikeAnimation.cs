...
public class BikeAnimation : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		if (ResetPanel == null)
		{
			ResetPanel = GameObject.FindWithTag("ResetPanel");
	...
	private void Update()
	{
		if (PlayerPrefs.GetInt("ResetApproved") == 1)
		{
			PlayerPrefs.SetInt("ResetApproved", 0);
			PlayerPrefs.SetInt("BikeRest", 3);
			crashesCount = 0;
			resetScript.ResetCar();
			ResetPanel.SetActive(value: false);
			gameController.currentState = GameControllerScript.GameState.isGamePlay;
			Time.timeScale = 1f;
		}
		if (timer != 0f)
		{
		...
		if (timer == 0f)
		{
			...
			if (BikeScript.crash)
			{
				...
				{
					ResetPanel.SetActive(value: true);
