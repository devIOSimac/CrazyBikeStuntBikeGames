...
namespace EVP
{
	public class VehicleRandomInput : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			{
				if (UnityEngine.Random.value < steerStraightRandom)
				{
					m_targetSteer = 0f;
				}
				else
				{
					m_targetSteer = UnityEngine.Random.Range(-1f, 1f);
				}
				m_nextSteerTime = Time.time + steerInterval + UnityEngine.Random.Range(0f - steerIntervalTolerance, steerIntervalTolerance);
			}
			if (Time.time > m_nextThrottleTime)
			{
				...
				{
					m_targetThrottle = UnityEngine.Random.value;
					m_targetBrake = 0f;
				}
				else if (magnitude < 0.5f)
				{
					m_targetBrake = 0f;
					m_targetThrottle = 0f - UnityEngine.Random.value;
				}
				else
				{
					m_targetThrottle = 0f;
					m_targetBrake = UnityEngine.Random.value;
				}
				m_nextThrottleTime = Time.time + throttleInterval + UnityEngine.Random.Range(0f - throttleIntervalTolerance, throttleIntervalTolerance);
			}
