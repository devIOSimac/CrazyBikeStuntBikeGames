...
public class MB3_TextureBaker : MB3_MeshBakerRoot
{
	...
	public IEnumerator CreateAtlasesCoroutine(ProgressUpdateDelegate progressInfo, CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = 0.01f)
	{
		...
		if (!_doMultiMaterial)
		{
			...
			for (int j = 0; j < objsToMesh.Count; j++)
			{
				...
				foreach (Material material in gOMaterials)
				{
					...
					{
						UnityEngine.Debug.LogWarning("Game object " + objsToMesh[j] + " does not use shader " + shader + " it may not have the required textures. If not small solid color textures will be generated.");
					}
				}
			}
		}
		...
		for (int i = 0; i < OnCombinedTexturesCoroutineAtlasesAndRects.Length; i++)
		{
			...
			UnityEngine.Debug.Log($"Creating atlases for result material {resMatToPass} using shader {resMatToPass.shader}");
	...
	public static void ConfigureNewMaterialToMatchOld(Material newMat, Material original)
	{
		if (original == null)
		{
			UnityEngine.Debug.LogWarning("Original material is null, could not copy properties to " + newMat + ". Setting shader to " + newMat.shader);
			return;
		}
	...
	private bool _ValidateResultMaterials()
	{
		...
		for (int k = 0; k < resultMaterials.Length; k++)
		{
			...
			for (int l = 0; l < mB_MultiMaterial.sourceMaterials.Count; l++)
			{
				...
				{
					UnityEngine.Debug.LogWarning("Source material " + mB_MultiMaterial.sourceMaterials[l] + " does not use shader " + shader + " it may not have the required textures. If not empty textures will be generated.");
				}
				if (hashSet2.Contains(mB_MultiMaterial.sourceMaterials[l]))
				{
					UnityEngine.Debug.LogError("A Material " + mB_MultiMaterial.sourceMaterials[l] + " appears more than once in the list of source materials in the source material to combined mapping. Each source material must be unique.");
					return false;
				}
